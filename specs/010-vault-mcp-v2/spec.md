...
| How does the Gate physically write? | ~~Wrap the Obsidian CLI (`cli_client.py`).~~ **REVERSED** — the CLI cannot cross the session-0/session-1 boundary, so the Gate shipped on `RestNoteIO` and `cli_client.py` was deleted 2026-09-10. REST writes still go through Obsidian, so the indexing rationale below holds. | Rob's explicit choice — guarantees Obsidian's indexing and plugins fire on every write. | Direct filesystem (deterministic but bypasses Obsidian index); hybrid. **Constitution III mitigation:** the CLI client is mocked in tests (the same rule the constitution already mandates for REST-backed tools — never a live instance), so the write path stays deterministically testable. |
