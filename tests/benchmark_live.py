...
def run_benchmark(vault_path: Path) -> None:
    print(f"vault-mcp Performance Benchmark")
    print(f"{'=' * 60}")
    ...
    print(f"  Timing:  {stats['min_ms']:.0f} / {stats['median_ms']:.0f} / {stats['max_ms']:.0f} ms (min/median/max)")
    print()
    ...
    for label, filters, scope in queries:
        stats = _time(lambda f=filters, s=scope: idx.find_notes_by_frontmatter(f, s))
        count = _count(stats["result"])
        print(f"  {label:35s} → {count:>4} results  {stats['median_ms']:7.1f} ms median")
    print()
    ...
    for stem in well_connected:
        ...
        print(f"  {stem:20s}  backlinks: {bl_count:>3} ({stats_bl['median_ms']:5.1f} ms)  outbound: {ol_count:>3} ({stats_ol['median_ms']:5.1f} ms)")
    print()
    ...
    ignores_path = vault_path / "System" / "Tools" / "Skills" / "vault-propagation" / "audit-ignores.md"
    if ignores_path.exists():
        ...
        print(f"  With audit-ignores:  {scoped_count} orphans  {stats_scoped['median_ms']:.0f} ms median")
    print()
    ...
    if glossary_path.exists():
        stats = _time(lambda: idx.tag_glossary_check(glossary_path), iterations=3)
    ...
    vs = stats["result"]
    print(f"  Total indexed: {vs['total_indexed']}, Total names: {vs['total_names']}")
    ...
    print(f"  Frontmatter-only: {fm_count} unique tags  {stats_fm_only['median_ms']:.1f} ms median")

    stats_with_body = _time(lambda: idx.all_tags(include_body=True), iterations=3)
    all_count = len(stats_with_body["result"])
    print(f"  With body tags:   {all_count} unique tags  {stats_with_body['median_ms']:.0f} ms median")
    print()
    ...
    for stem in read_targets:
        ...
        r = stats["result"]
        link_count = len(r.get("outbound_links", [])) if isinstance(r, dict) else 0
        print(f"  {stem:20s}  {link_count:>3} outbound links  {stats['median_ms']:7.1f} ms median")
    print()

    # --- Summary ---
    print(f"{'=' * 60}")
    print("Summary")
    print(f"  Vault size: {reindex_result['indexed']} indexed / {reindex_result['names']} names")
