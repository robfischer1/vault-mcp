...
class FakeVault:
    ...
    def list_notes(self, directory: str = "", *, recursive: bool = True) -> list[str]:
        return [
            p
            for p in self.store
            if not directory or p == f"{directory}.md" or p.startswith(directory + "/")
        ]
...
_BASE = ["title: t", "created: 2026-05-30", "author_type: ai", "author_level: ai-assisted"]
...
class TestScan:
    def test_clean_note_not_reported(self):
        gate, _ = _gate()
        gate.create_note(
            title="Clean", note_type="note", pillar="Knowledge", body="x", created="2026-05-30"
        )
    ...
    def test_deprecated_key_and_dead_key_reported(self):
        gate, vault = _gate()
        vault.store["Knowledge/Notes/d.md"] = _note([*_BASE, "id: legacy-1", "sub_type: junk"])
        report = gate.audit("Knowledge", resolve=False)
...
class TestHeal:
    def test_heals_deprecated_key_status_and_dead_key(self):
        ...
        assert "identifier: legacy-7" in front  # id -> identifier (value preserved)
